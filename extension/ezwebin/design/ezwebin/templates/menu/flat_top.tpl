    ...
    {if $top_menu_items_count}
       {foreach $top_menu_items as $key => $item}
            ...
            {else}
				...
				{if eq($item.name,"Contact us")}
				{else}
			
                <li id="node_id_{$item.node_id}"{if $item_class} class="{$item_class|implode(" ")}"{/if}><div><a href={if eq( $ui_context, 'browse' )}{concat("content/browse/", $item.node_id)|ezurl}{else}{$item.url_alias|ezurl}{/if}{if $pagedata.is_edit} onclick="return true;"{/if}><span>{$item.name|wash()}</span></a></div></li>
				{/if}
            {/if}
          {/foreach}
