...
<div class="box-ml"><div class="box-content">
    ...
    <div id="eznwinst_info">
        <div id="eznwinst_info_start" class="eznwinst_info">
            ...
            <p>{'This is a simplified installer, it does not support:'|i18n( 'design/admin/network' )}<br />
               {'* Cluster setup (including dfs)'|i18n( 'design/admin/network' )}<br />
               {'* Multi DB setup'|i18n( 'design/admin/network' )}
            </p>
        ...
        <div id="eznwinst_info_finalize" class="eznwinst_info hide">
            ...
            <p>Last thing to do is to setup cronjobs for eZ Network, correct php path if needed and install the following in your crontab:
            <pre>
...
{$rnd_minute|sum( 11 )} {$rnd_hour} * * *  cd $ezpath && $phpbin runcronjobs.php -q patch >/dev/null 2>&1
...
jQuery( function( $ )
{
    ...
    $('#eznwinst_info_install_nw_key_btn').click(function()
    {
        ...
        $.post( $.ez.root_url + 'network/install', "doAction=install&nwKey=" + inp.val(), fn.installResponse, 'json');
    ...
    fn.sanity = function( )
    {
        ...
        $.post( $.ez.root_url + 'network/install', "doAction=md5", fn.sanityResponse, 'json');
        $.post( $.ez.root_url + 'network/install', "doAction=env", fn.sanityResponse, 'json');
    };
    ...
    fn.installResponse = function( data, textStatus, req )
    {
        ...
        else if ( fn.installPostActions === undefined )
        {
            ...
            $.post( $.ez.root_url + 'network/install', "doAction=snapshoot", fn.installResponse, 'json');
            $.post( $.ez.root_url + 'network/install', "doAction=sync", fn.installResponse, 'json');
        }
    };
