<?php
//
// Definition of eZNetUtils class
//
// Created on: <06-Jul-2005 15:52:54 hovik>
//
...
class eZNetUtils
{
    ...
    */
    ...
    {
        $updateFieldList = array( true => array( 'mode' => 'fmode' ),
                                  false => array( 'fmode' => 'mode' ) );
    ...
    */
    static function createTable( $input )
    {
        $tableSchema = false;
        if ( is_object( $input ) &&
             ( $input instanceOf eZPersistentObject ) )
        {
        ...
        foreach( $tableSchema as $key => $definition )
        {
            ...
            else
            {
                ...
                $autoIncFieldList = $params;
                // if oracle we should pass autoinc fields to get proper defenition from db
                if ( $isOracle )
                {
                    $autoIncFieldList = array();

                    foreach ( $definition['fields'] as $field => $fieldData )
                    {
                        if ( $fieldData['type'] == 'auto_increment' )
                        {
                            $autoIncFieldList[$tableName . '.' . $field] = 'auto_increment';
                        }
                    }
                }

                $fields = $dbSchema->fetchTableFields( $lookingTableName, $autoIncFieldList );
