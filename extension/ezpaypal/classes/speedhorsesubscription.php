...
class SpeedHorseSubscription
{
    ...
    */
    ...
    function SpeedHorseSubscription()
    {
    ...
    */
    function activate( $orderID, $currentUser )
    {
		...
		//print $subscriptionGroupNodeID;
		$subscriptionGroupNodeIDarr = array();
		
		$subscriptionGroupNodeIDarr=explode(';',$subscriptionGroupNodeID);
		
		
		
		
		//print_r($subscriptionGroupNodeIDarr);
        ...
        if ( $this->isSubscriptionProduct( $order ) == true )
        {
            ...
            foreach ( $order->productItems() as $product )
            {
                ...
                if ( isset( $productObjectDatamap[$subscriptionProductAttributeName] ) )
                {
					...
					$productAttributeNumber = $productObjectDatamap['product_id'];
					$PID = $productAttributeNumber->content();
					
					
					if($PID=="NFS"){
						$subscriptionGroupNodeID=$subscriptionGroupNodeIDarr['0'];
						//print "<br />".$subscriptionGroupNodeID;
					}
					elseif($PID=="NPS"){
						$subscriptionGroupNodeID=$subscriptionGroupNodeIDarr['1'];
					}
					elseif($PID=="NSS"){
						$subscriptionGroupNodeID=$subscriptionGroupNodeIDarr['2'];
					}
					elseif($PID=="IFS"){
						$subscriptionGroupNodeID=$subscriptionGroupNodeIDarr['3'];
					}
					elseif($PID=="IPS"){
						$subscriptionGroupNodeID=$subscriptionGroupNodeIDarr['4'];
					}
					elseif($PID=="ISS"){
						$subscriptionGroupNodeID=$subscriptionGroupNodeIDarr['5'];
					}
					$mYear=$days/365;
					//$mDays=$days%365;
					$this->totalTime=$mYear." year/years";
    ...
    */
    function degradeExpiredSubscriptionUsers( )
    {
		...
		$treeContentObj= new eZContentObjectTreeNode;
		
        $users =$treeContentObj->subTree( array('ClassFilterArray' => array( $classID ),
    ...
    */
    function sendMembershipActivationNoticeToUserEmail( $userObject )
    {
       ...
       // $lastName = $attributeLastName->content();
        ...
        $tpl->setVariable( 'user_first_name', $firstName );
        $tpl->setVariable( 'user_last_name', $lastName );
		$tpl->setVariable( 'period', $this->totalTime );
        ...
        $body = $tpl->fetch("design:user/subscription_activation_email_notification.tpl");
        $results = $this->sendNotificationEmail( $to, $subject, $body );
