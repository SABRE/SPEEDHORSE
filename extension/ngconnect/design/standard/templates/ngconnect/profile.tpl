...
<div class="ngconnect-profile">
	...
	{if $forced_redirect|not}
		<p>{'If you already have a regular account, input your details under <strong>"Login to existing account"</strong> section.'|i18n('extension/ngconnect/ngconnect/profile')}</p>
		<p>{'If you do not have a regular account, enter your desired username and password under <strong>"Create new account"</strong> section.'|i18n('extension/ngconnect/ngconnect/profile')}</p>
		<p>{'If you do not wish to create a regular account, simply click the <strong>"Skip"</strong> button below, and we won\'t bother you with this again. An email with the Username and New Password will be sent to your registered email address.'|i18n('extension/ngconnect/ngconnect/profile')}</p>
		{else}
	...
	<div class="block">
		<form action={'ngconnect/profile'|ezurl} method="post">
			<label>{'Email'|i18n('extension/ngconnect/ngconnect/profile')}</label><div class="labelbreak"></div>
	...
	{if $forced_redirect|not}
		...
		<div class="block">
			<form action={'ngconnect/profile'|ezurl} method="post">
				<label>{'Email'|i18n('extension/ngconnect/ngconnect/profile')}</label><div class="labelbreak"></div>
				<input class="halfbox" type="text" size="10" name="data_user_login" value="{cond(and(ezhttp_hasvariable('data_user_email', 'post'), ezhttp('data_user_email', 'post')|count), ezhttp('data_user_email', 'post'), $network_email)}" tabindex="5" />

				<!--<label>{'E-mail'|i18n('extension/ngconnect/ngconnect/profile')}</label><div class="labelbreak"></div>-->
				<input class="halfbox" type="hidden" size="10" name="data_user_email" value="{cond(and(ezhttp_hasvariable('data_user_email', 'post'), ezhttp('data_user_email', 'post')|count), ezhttp('data_user_email', 'post'), $network_email)}" tabindex="6" />

				<label>{'Password'|i18n('extension/ngconnect/ngconnect/profile')}</label><div class="labelbreak"></div>
				<input class="halfbox" type="password" size="10" name="data_user_password" value="" tabindex="7" />

				<label>{'Confirm password'|i18n('extension/ngconnect/ngconnect/profile')}</label><div class="labelbreak"></div>
