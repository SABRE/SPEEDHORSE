...
class eZAuthorizeGateway extends eZCurlGateway
{
    ...
    */
    ...
    {
        $ini = eZINI::instance( 'ezauthorize.ini.append.php' );
	...
	eZDebug::writeDebug( $xmlDoc );
        ...
        if ( $ini->variable( 'eZAuthorizeSettings', 'StoreTransactionInformation', 'ezauthorize.ini.append.php') == true  )
        {
    ...
    */
    function doCURL( &$process )
    {
        ...
        else
        {
			...
			if(isset($response['Customer ID']) && isset($response['Invoice Number'])){
				$orderID=$response['Invoice Number'];
			  $orderIDupdate=$response['Invoice Number'];
			  ...
			  $startStatusCode=3;
				...
				// Create a new object
				include_once("extension/ezauthorize/classes/speedhorsesubscription.php" );
