	...
	<div style="float:left;">
    	<div id="content" style="z-index:9999;"><!--start div#content-->
			...
			<div id="the_post">
                ...
                <div class="post-info"><h3><span style="font-weight:bold;">{$node.data_map.publication_date.data_int|datetime( 'mydate', '%F')} {$node.data_map.publication_date.data_int|datetime( 'mydate1', '%d' )}, {$node.data_map.publication_date.data_int|datetime( 'mydate4', '%Y' )}</span>&nbsp;&nbsp;|&nbsp;&nbsp;<a href="/userdetail/list/{$node.object.current.creator.id}">{$node.object.current.creator.name|wash()}</a></h3></div>
				...
				{else} 
                {def $valarticle=fetch( 'content', 'object', hash( 'object_id', $rvalue ))}
               	<div class="float_img_left">{attribute_view_gui attribute=$valarticle.data_map.image image_class=blog_inner_img}</div>
                {/if}
