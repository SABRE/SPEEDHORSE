				 ...
				 <h2>{$child.name}</h2><br /><br />
					...
					<div id="magazine_items">				
									...
									{foreach fetch_alias( 'children', hash( 'parent_node_id', $child.node_id) ) as $childnew }
										...
										{/if}
										<a href="http://collegeyardart.com/magazines/{attribute_view_gui attribute=$childnew.data_map.magazine_link}" target="_blank">{attribute_view_gui attribute=$childnew.data_map.big_image}</a>
											</div>
										{set icol=$icol|inc}
									{/foreach}	
					</div>	
