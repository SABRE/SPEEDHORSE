							...
							{else}
								...
								<ul style="padding-left:5px; padding-top:2px;margin-right:10px;" class ="mainmagazine">
									...
									<li>
										...
										<a href="#" class="myleftclass" id="ul_{$myemagazine.node_id}_div">{$myemagazine.name}</a>
									
										
									{if $children_count}
										<ul style="padding-left:15px; padding-top:2px;padding-bottom:2px;" class="subemagazine subemagazineactive" id="subul_{$myemagazine.node_id}_div">
									{foreach fetch_alias( 'children', hash( 'parent_node_id', $myemagazine.node_id) ) as $child }
										
											<li class="subemagazineli subemagazineactive" id="li_{$child.node_id}_{$dcol}" ><a href="#" style="margin-left:20px;">{$child.name}</a>																			           										<ul>
													{foreach fetch_alias( 'children', hash( 'parent_node_id', $child.node_id) ) as $childnew}
													<li class="subemagazineli subemagazineactive" id="li_{$childnew.node_id}_{$dcol}" ><a href="#" style="margin-left:35px;">{attribute_view_gui attribute=$childnew.data_map.small_image }{$childnew.name}</a></li>
													{/foreach}
												</ul>
											</li>
										{/foreach}
