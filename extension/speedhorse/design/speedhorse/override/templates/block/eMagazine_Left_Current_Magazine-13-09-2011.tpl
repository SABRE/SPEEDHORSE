					...
					{foreach fetch_alias( 'children', hash( 'parent_node_id', $myemagazine.node_id ) ) as $myemagazine_sub reverse}
					 ...
					 {set $children_count=fetch_alias( 'children_count', hash( 'parent_node_id', $myemagazine_sub.node_id) )}
								...
								{if $children_count}
									...
									{if eq($col,1)}
										<li class="subemagazineli subemagazineactive" id="li_{$child.node_id}_{$dcol}" ><a href="#" style="margin-left:25px;">{$child.name}</a></li>
									{else}
										<li class="subemagazineli" id="li_{$child.node_id}_{$dcol}" ><a href="#" style="margin-left:25px;">{$child.name}</a></li>	
									{/if}
					...
					<div id="aremagazine_wrap" style="background:#fffaf4;height:auto;" class="newaremagazine_wrap">
							...
							{set $children_count=fetch_alias( 'children_count', hash( 'parent_node_id', $myemagazine.node_id) )}
								...
								{if $children_count}
									...
									{if eq($col,1)}
										<li class="" id="li_{$child.node_id}_{$dcol}" ><a href="#" style="margin-left:25px;">{$child.name}</a></li>
									{else}
										<li class="" id="li_{$child.node_id}_{$dcol}" ><a href="#" style="margin-left:25px;">{$child.name}</a></li>	
									{/if}
													{if eq($col,1)}
