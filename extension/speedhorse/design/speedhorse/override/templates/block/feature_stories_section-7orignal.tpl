					...
					{foreach $valid_nodes as $key => $feature_stories}
						{if gt($col,2)}
							...
							{/if}
								...
								<td>
								<div class="post">
									<div style="float: left;margin-right: 10px;">{attribute_view_gui attribute=$feature_stories.data_map.image image_class='artical_img' href=$feature_stories.url_alias|ezurl()}</div>
