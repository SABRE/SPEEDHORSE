				...
				<div id="blogs_preview">
                    ...
                    {/section}	
                     ...
                     {def $valblog=fetch( 'content', 'object', hash( 'object_id', $myobjid ))}
                        <a id="blogs_preview_ad_link" href={attribute_view_gui attribute=$valblog.data_map.company_website} target="_blank"><img src="/extension/speedhorse/design/speedhorse/images/sponsor_blog.png" />{attribute_view_gui attribute=$valblog.data_map.sponsores_image image_class=blogsponsor_img }</a>
...
{def $valarticle=""}
					...
					{foreach $valid_nodes as $key => $blog}
                    	...
                    	{set $my_node1=fetch( 'content', 'node', hash( 'node_id', $blog.node_id ) )}
					...
					<img class="post_thumbnail" src="../images/blog_author.jpg" alt="post_thumbnail">
							...
							<div style="float: left;margin-right: 10px;">{attribute_view_gui attribute=$valarticle.data_map.image image_class='blog_img' href=$blog.url_alias|ezurl()}</div>
