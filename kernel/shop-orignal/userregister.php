...
if ( $user->isLoggedIn() )
{
	...
	if($key=="address")
		{
		$street1=$value->attribute('data_text');
		}
	if($key=="zip")
		{
		$zip=$value->attribute('data_text');
		}
	if($key=="city")
		{
		$place=$value->attribute('data_text');
		}	
	if($key=="state")
		{
	...
	if($key=="country")
		{
		//print "<br>country".$value->attribute('data_text');
		$country=$value->attribute('data_text');
		}
	}
}
